...
SELECT 
	...
	, LoanPurpose				= CASE WHEN LK.LoanPurposeID = 6 THEN 'Purchase' --changed for Purchase Criteria
									ELSE 'Other' END
	, EmployeeLoanFlg
	, SelfEmployflg
	, ReverseFlg
...
AND LK.DeleteFlg = 0
AND LTF.LoanPurposeID = 6  --Changed to 6 for Purchase Criteria
AND LK.ReverseFlg = 0
...
WHERE (
	(
		...
		AND S20.LoanChannel NOT LIKE '%QLMS%'		--added for Purchase criteria
		AND S20.LoanChannel NOT LIKE '%Relocation%'	--added for Purchase criteria
		AND (
