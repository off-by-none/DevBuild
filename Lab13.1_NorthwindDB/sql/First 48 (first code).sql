...
Select s20.*
              , ClientReady.StatusDtID
              , ClientReady.StatusTmID
...
FROM #stat20loans s20
       ...
       OUTER APPLY(
                           ...
                           FROM QLODS..LKWDTrackingItemFact tif (NOLOCK)
                                  INNER JOIN QLODS..LKWDTrackingItemStatusDim sd (NOLOCK) ON sd.StatusID = tif.StatusID
                           WHERE tif.Loannumber = s20.LoanNumber
                                  AND tif.trackingitemid = 5659 --TI 5957 All Client Conditions Have Been Cleared
                                  AND sd.StatusDescription like 'cleared'
                           ORDER BY tif.StatusDtID Desc, tif.StatusTmID DESC
                           ) ClientReady


WHERE isnull(TI8107.StatusDescription, 'cancelled') like 'cancelled'
       AND isnull(TI1727.StatusDescription, 'cancelled') like 'cancelled'
