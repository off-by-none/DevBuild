...
IF OBJECT_ID('tempdb..#FirstClientReady','U') IS NOT NULL
       DROP TABLE #FirstClientReady

SELECT C.*
	, FirstClientReadyDtID			= FCR.StatusDtID
	, FirstClientReadyTmID			= FCR.StatusTmID
INTO #FirstClientReady
FROM #ClientReady
C
OUTER APPLY (
                ...
                FROM QLODS.dbo.LKWDTrackingItemFact
                TIF (NOLOCK)
                WHERE TIF.LoanNumber = C.LoanNumber
                AND TIF.TrackingItemID = 5659 -- All Client Conditions Cleared
                AND TIF.StatusID = 67
                AND TIF.DeleteFlg = 0
				AND TIF.StatusDtID >= C.CreateDtID
                ORDER BY TIF.StatusDtID ASC
                        , TIF.StatusTmID ASC
            ) FCR

IF OBJECT_ID('tempdb..#LastClientReady','U') IS NOT NULL
	DROP TABLE #LastClientReady

SELECT C.*
	, LastClientReadyDtID			= LCR.StatusDtID
	, LastClientReadyTmID			= LCR.StatusTmID
INTO #LastClientReady
FROM #FirstClientReady
C
OUTER APPLY (
...
OUTER APPLY (
				...
				AND LTF.DeleteFlg = 0
				AND LTF.TransDtID >= Q.CreateDtID
...
OUTER APPLY (
				...
				AND TIF.DeleteFlg = 0
				AND TIF.StatusID = 11
				AND TIF.StatusDtID >= S40.CreateDtID
				ORDER BY TIF.StatusDtID ASC
					, TIF.StatusTmID ASC
			) FirstRFSOP
...
OUTER APPLY (
				...
				D
				WHERE D.DateID BETWEEN 20170301 AND 20180519
...
SELECT *
INTO BISandboxWrite.jwoz.First48Metrics
FROM #RawData
...
SELECT Pilot
	...
	, FI_Suspended			= COUNT(CASE WHEN FolderIncompleteDate IS NOT NULL AND SuspensePre35 > 0 THEN LoanNumber END)*1.0/COUNT(LoanNumber)
...
/*****************************************************************************************************
 Time in Loan: Job Group Level With Historical Job Title
*****************************************************************************************************/
SELECT 
  #RawData.Pilot
--, [Job Grouping]
--, jtdim.JobTitle
, [Total Time in Loan (hours)]		= SUM(sf.SessionDuration)/3600.0
, [Distinct Loans Count]			= count(distinct #RawData.LoanNumber)
--, [Distinct Session Count]			= count(distinct sf.SessionFactID)

FROM #RawData
	INNER JOIN BILoan.Loan.SessionFact sf ON sf.LoanNumber = #RawData.LoanNumber
	...
	CROSS APPLY (
					SELECT [Job Grouping] = CASE WHEN jtdim.JobTitle IN ('Quality Assurance Analyst', 'Sr Qual. Assurance Anlst.') THEN 'Quality Assurance'
													WHEN jtdim.JobTitle = 'Income Specialist' THEN 'Income Specialist'
													WHEN jtdim.JobTitle = 'Operations Specialist' THEN 'Ops Specialist'
													WHEN jtdim.JobTitle = 'Associate Underwriter' THEN 'Associate Underwriter'
													WHEN jtdim.JobTitle IN ('Executive Underwriter QL'
																		, 'PC Underwriter'
																		, 'Purchase Agreement UW'
																		, 'Restructure Underwriter' --Waiting for response on if this should be included In LRT Underwriter
																		, 'Sr. Underwriter QL'
																		, 'Triple Crown Underwriter'
																		, 'Underwriter QL') THEN 'Underwriter'
													WHEN jtdim.JobTitle IN ('Executive FSO Underwriter'
																		, 'FSO Underwriter'
																		, 'Pres.Club FSO Underwriter'
																		, 'Sr. FSO Underwriter'
																		, 'Triple Crown FSO UW') THEN 'Final Signoff Underwriter'
													ELSE 'Problem' END
				)jg
WHERE 1=1
	AND jg.[Job Grouping] <> 'Problem'
