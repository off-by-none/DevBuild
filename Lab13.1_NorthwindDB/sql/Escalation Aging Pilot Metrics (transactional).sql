/***********************************************************************************
Aging Loan Metrics for Escalation Team (Transactional Group)
Stakeholders: Lindsay Villasenor, Jeanine Taylor, Debra Abrams, Eric Birk
Updated - 09.12.2017 - Brandon Brewer

A transactional table of the New Aging Escalation Process.
	*Also included are some metrics

This query grabs all the loans that enters the process and all their 'events'.
Events defined as:
	*Scrubbed
	*Help Requested
	*Scrubbed Cleared
	*Client Ready
	*Suspended
	*Exits the process (close/fallout)

The bottom portion of the query is used to pull metrics.
***********************************************************************************/
Set Transaction Isolation Level Read Uncommitted

/***********************************************************************************
Date Range
First Day of new process was April 13 (to the end of time).
***********************************************************************************/
...
, [StatusDescription]	= CASE WHEN TI.Dt IS NOT NULL AND SUS.Sus IS NOT NULL THEN
							   CASE WHEN TI.Dt < Sus.Sus THEN 'Scrubbed' ELSE 'Suspended' END
							   WHEN Sus.Sus IS NULL THEN 'Scrubbed' ELSE 'Suspended' END 
...
FROM #bp
	...
	INNER JOIN QLODS..LKWDTrackingItemFact tif ON tif.LoanNumber = #bp.LoanNumber
		...
		AND tif.StatusDtID >= @StartDtId  --Only return statuses after first CBU
	INNER JOIN QLODS..LKWDTrackingItemStatusDim tisd ON tisd.StatusID = tif.StatusID
...
FROM #bp
	CROSS APPLY (
					...
					WHERE ltf.LoanNumber = #bp.LoanNumber
						...
						AND ltf.TransDateTime > #bp.StatusDateTime --Suspended on or after 4/13
				)suspended
...
WHERE L.ClosingID < CAST(CONVERT(VARCHAR(8),CONVERT(DATE,GETDATE()),112) AS INT)
	AND SD.StatusKey > 35
...
FROM #bp
	CROSS APPLY (
					...
					WHERE tif.LoanNumber = #bp.LoanNumber
						...
						AND tif.DeleteFlg = 0
						AND tif.StatusDateTime > #bp.StatusDateTime
...
SELECT * FROM #final ORDER BY #final.LoanNumber, #final.rowNum
...
SELECT DISTINCT #final.LoanNumber 'Scrubbed then Help Requested'
FROM #final
WHERE #final.StatusDescription = 'Help Requested'
...
SELECT count(DISTINCT #final.LoanNumber) 'Scrubbed then Help Requested'
FROM #final
WHERE #final.StatusDescription = 'Suspended' AND #final.rowNum = 1