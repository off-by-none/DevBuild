...
DECLARE @startDt AS INT	= 20170500
DECLARE @endDt AS INT = 20170599
...
, lpd.LoanPurpose		'Loan Purpose'
, em.FullNameFirstLast	'FSO UW'
, [Is 4561]				= CASE WHEN tif.LoanNumber IS NOT NULL THEN 1 ELSE 0 END
, em.CommonID
, [Units]				=  CASE WHEN CONCAT(pb.JumboFlg, lpd.LoanPurpose, pb.ProductBucket) = '0PurchaseConventional' THEN 1.75
								...
								WHEN CONCAT(pb.JumboFlg, lpd.LoanPurpose, pb.ProductBucket) = '0RefinanceConventional' THEN
										CASE WHEN tif.LoanNumber IS NOT NULL THEN 0.5 ELSE 1 END
								WHEN CONCAT(pb.JumboFlg, lpd.LoanPurpose, pb.ProductBucket) LIKE '0RefinanceFHA%' THEN
										CASE WHEN tif.LoanNumber IS NOT NULL THEN 0.5 ELSE 1.25 END
								WHEN CONCAT(pb.JumboFlg, lpd.LoanPurpose, pb.ProductBucket) LIKE '0RefinanceVA%' THEN
										CASE WHEN tif.LoanNumber IS NOT NULL THEN 0.5 ELSE 1.25 END
...
SELECT * FROM #closingdata WHERE #closingdata.[FSO UW] LIKE '%Dawn%'
